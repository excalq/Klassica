<?php
  ...
  // Verify the message's sender as current user
  if ($_GET['ut'] == 'send' && $_GET['deletemsg']) {
    ...
    if ($del_message_id > 0) {
      ...
      $kmsg->delete_message($del_message_id, $user_id, 'sender');
    }
  }
  ...
  if ($_POST['message2_order_by'] && $_POST['message2_order_dir']) {
    // switch order
    $order_direction = ($_POST['message2_order_dir'] == 'DESC')? 'ASC' : 'DESC';
    // set order_by
    $kmsg->set_orderby($_POST['message2_order_by'], $order_direction);
  }
  
  // advance page on request (start at page 0)
  $page = $_POST['message2_page'];
  if ($page)
    $kmsg->set_viewpage($page);
  
  // Fetch array of messages
  $messages_arr = $kmsg->get_messages_by_sender($user_id);
...
<div>
  <h3 class="titleLabel table_title">Messages: Sent</h3>
  <div id="table_container">
      <table class="messagestable">
        <tr class="header">
          <th class="th_delete"></th>
          <th class="th_from">To</th>
...
<?php
  if ($kmsg->get_result_count() < 1) {
    ...
    foreach($messages_arr as $msg)
    {
      ...
      echo "\t<td class=\"delete\"><a href=\""._SITE_PATH."/myklassica/?deletemsg={$msg['id']}&ut=send\" onclick=\"return confirm('Are you sure you want to delete this message?');\">
...
?>
    ...
    <form name="message2_page" method="post" action="<?php echo _SITE_PATH."/myklassica/"; ?>">
      <select type="select" name="message2_page">
        <?php
          ...
          for ($i = 1; $i<=$kmsg->get_page_count(); $i++) {
            ...
            $selected = ($_POST['message2_page'] == $i)? " selected=\"selected\"" :  "";
