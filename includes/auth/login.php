...
require_once ("../klassica.php");

// WWU's official authentication script, provided by web services - Local copy updated 2008-03-18
require_once ('wwuauth.php');

include ("../classes/write_logfile.php");
$log = new write_logfile("auth_log.txt");
...
// If login form was submitted, authenticate against WWU's auth system, then log into klassica
if ($_POST['kauth'] && $_POST['kauthuname'] && $_POST['kauthtoken']) {
  $username = $_POST['kauthuname'];
  $password = $_POST['kauthtoken'];
  
  $_SESSION['auth']['kauthusername'] = $username;
  $_SESSION['auth']['kauthtoken'] = md5($password); // saved in session for using after WWU auth check
...
// WWU Authentication was unsuccessful, so try logging into a local Klassica account
if ($_SESSION['auth']['kauthusername'] && $_GET['error']) {
  $username = $_SESSION['auth']['kauthusername'];
  $password = $_SESSION['auth']['kauthtoken'];
  
  unset($_SESSION['auth']['kauthusername'], $_SESSION['auth']['kauthtoken']);
  ...
  $local_auth = $kuser->authenticate_user($username, $password, false);
  ...
  if ($local_auth === 0)
  {
    ...
    // Neither WWU nor Klassica could not authenticate this user
